...
def champ_id_remap():
    ...
    with open('./backend/metadata/id_to_key.json', 'r') as fp:
...
def make_idf_table():
    ...
    for i in range(12):
        batch_file = './backend/data_batch/batch2_' + str(i) + '.json'
    ...
    for i in range(4):
        batch_file = './backend/data_batch/batch3_' + str(i) + '.json'
        with open(batch_file, 'r') as fp:
            batch_data = json.load(fp)
    ...
    for i in range(3):
        batch_file = './backend/data_batch/batch4_' + str(i) + '.json'
        with open(batch_file, 'r') as fp:
            batch_data = json.load(fp)
...
def global_win_rate():
    ...
    with open('./backend/data_batch/userbatch.json', 'r') as fp:
...
def champion_similarity_csv_to_json():
    ...
    champ_node_file = open('./backend/datasets/allchampions_nodes.csv', 'r')
    champ_node_reader = csv.reader(champ_node_file, delimiter=',')
    node_column = next(champ_node_reader)

    champ_edge_file = open('./backend/datasets/allchampions_edges.csv', 'r')
    ...
    with open('./backend/datasets/item_vectors_weights.json', 'w') as f:
