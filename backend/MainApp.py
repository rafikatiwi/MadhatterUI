...
with open('./backend/metadata/name_to_key.json', 'r') as fp:
...
class ChampionRecommender():
    def __init__(self):
        ...
        self.user_encoder.load_state_dict(torch.load('./backend/trained_model/user_encoder_augmented.pth', map_location = 'cpu'))

        self.item_encoder = AutoEncoder(143, 8)
        self.item_encoder.load_state_dict(torch.load('./backend/trained_model/item_encoder2.pth', map_location = 'cpu'))

        self.predictor = Predictor(user_len=12, item_len=8, hidden_unit=10).eval()
        self.predictor.load_state_dict(torch.load('./backend/trained_model/predictor_last_m.pth', map_location = 'cpu'))
        
        self.remapped_champ_id = champ_id_remap()
        self.global_win_rate = global_win_rate() 
        
        with open('./backend/datasets/item_vectors_tf_idf.json', 'r') as fp:
            self.item_vectors = json.load(fp)        
        
        with open('./backend/metadata/user_idf_table.json', 'r') as fp:
