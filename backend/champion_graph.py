...
def create_remapped_id_node():
    ...
    with open('./backend/metadata/id_to_key.json', 'r') as f:
        original_champ_id_dict = json.load(f)

    champion_node_old_file = open('./backend/datasets/graph_data/allchampions_old_nodes.csv', 'r')
    champion_node_old_reader = csv.reader(champion_node_old_file, delimiter=',')
    champion_node_file = open('./backend/datasets/graph_data/allchampions_nodes.csv', 'w')
...
class ChampionGraph():
    def __init__(self, user_name):
        ...
        with open('./backend/metadata/key_to_id.json', 'r') as f:
    ...
    def __get_champion_node(self):
        ...
        try:
            champ_node_file = open('./backend/datasets/graph_data/allchampions_nodes.csv', 'r')
    ...
    def __add_champion_edge(self):
        ...
        champ_edge_file = open('./backend/datasets/graph_data/allchampions_edges.csv', 'r')
    ...
    def _get_user_tf_idf(self):
        ...
        user_tf_idf = generate_user_vector(2, batch_data=user_data, to_file=False)[self.user_name]
